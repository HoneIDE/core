/**
 * Google Vertex AI adapter — Gemini models via Vertex AI.
 */
import type {
...
export class VertexAdapter implements AIProviderAdapter {
  ...
  buildRequest(req: AICompletionRequest, config: AIProviderConfig) {
    const baseUrl = config.baseUrl ?? 'https://us-central1-aiplatform.googleapis.com';
    const project = config.projectId ?? 'my-project';
    const location = config.region ?? 'us-central1';
    const action = req.stream ? 'streamGenerateContent' : 'generateContent';
    const url = `${baseUrl}/v1/projects/${project}/locations/${location}/publishers/google/models/${req.model}:${action}`;
  ...
  buildTestRequest(config: AIProviderConfig) {
    ...
    return {
      method: 'POST',
      url: `${baseUrl}/v1/projects/${project}/locations/${location}/publishers/google/models/gemini-2.0-flash:generateContent`,
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${config.apiKey ?? ''}`,
      },
